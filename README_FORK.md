...
**Version 0.8.19 is the latest stable release for both distributed packages:**

- Windows: `install_sieve_0.8.19.exe`
- Thunderbird: `sieve-0.8.19-cram-md5.xpi`
...
- Version `0.8.11` applies **Run Sieve now** to the explicitly selected Inbox
  ...
  maintainer scripts.
- Version `0.8.12` creates missing literal `fileinto` destinations before
  **Run Sieve now** and displays the server's returned action report. The Inbox
  adds a **Spam** action which marks and moves the exact selected message to
  Junk and queues authenticated Rspamd training. The shortened selection
  heading and a per-message context menu make the table more compact. The rule
...
- Version `0.8.14` opens the Inbox rule modal immediately with visible loading
...
- Version `0.8.16` fixes the empty Inbox rule modal. Message headers and the
  generated rule source are rendered before ManageSieve is connected, similar
  rules show an explicit result, and the complete editor now exposes the
  synchronized graphical **Script** and **Source** tabs. Required capabilities
  are enabled in the graphical parser, while a parser or connection error
  leaves the source visible with an actionable message.
- Version `0.8.17` prevents similar-rule inspection from remaining at its
  loading message. The generated graphical rule is rendered before connecting,
  the redundant account-wide script listing was removed, the current loading
  phase is shown, and stalled server operations end with a visible timeout.
- Version `0.8.18` fills the Inbox rule editor's target-script selector before
  reading individual script bodies and removes the non-canceling overall timer
  which could leave later source requests waiting behind abandoned work. The
  active script is inspected first, closing the modal stops further inspection
  requests, and a failed individual inspection no longer empties the selector.
  The Windows application also adds a visible **Close** action beside
  **Settings**; it and the native window close control both check open editors,
  disconnect ManageSieve sessions, and terminate the Electron process.
- Version `0.8.19` removes the implicit `INBOX` destination from the Inbox rule
  editor. Opening **Create Sieve Rule** produces no `fileinto` rule until the
  user explicitly selects an existing mailbox or enters a new mailbox name.
  Lint and Save remain disabled until that choice is applied. The dialog no
  longer calls the generated rule a template and explains that saving appends
  to the selected Sieve script without overwriting `INBOX` or another mailbox.
  A newly entered mailbox continues to use `fileinto :create` so it can be
  created on the first matching delivery when the server supports `mailbox`.
...
[`releases/sieve-0.8.19-cram-md5.xpi`](releases/sieve-0.8.19-cram-md5.xpi).
...
[`releases/install_sieve_0.8.19.exe`](releases/install_sieve_0.8.19.exe).
